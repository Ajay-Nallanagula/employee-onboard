...
const EmployeePersonalInformationSchema = object().shape({
  ...
  mobileNumber: number()
    .typeError("should be a 10-digit number")
    .test("Mobile Number", "Enter a valid Mobile number", (value) => {
      const mobileRegEx = /^\d{10}$/;
      return !value || mobileRegEx.test(value);
    }),
  alternateNumber: number()
    .typeError("should be a 10-digit number")
    .test('Mobile and Alternate Match', 'Mobile Number and Alternate Number should be different', function(value) {
      return this.parent.mobileNumber !== value;
    })
    .test("Mobile Number", "Enter a valid Mobile number", (value) => {
      const mobileRegEx = /^\d{10}$/;
      return !value || mobileRegEx.test(value);
